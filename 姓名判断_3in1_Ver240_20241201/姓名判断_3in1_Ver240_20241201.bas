...
'Sprite 定義                                           *
'*****************************************************
gload "./config/gazo/"+"selection.png",1,10,100
gload "./config/gazo/"+"selection.png",1,10,200
gload "./config/gazo/"+"selection.png",1,10,300
gload "./config/gazo/"+"selection.png",1,10,400
gload "./config/gazo/"+"selection.png",1,10,500
...
'*****************************************************
dim select$(3)
open "config/Selection/Selection.dat" for input as #2
for i=0 to 2
input #2,select$(i)
next i
close #2
...
'1行目だけ読み込む                                           *
'*****************************************************
open "config/Kanji_data/Mojidata_ReiIden20200911.dat" for input as #2
for i=0 to 26
input #2,buf_lines$(i)
next i
close #2
...
'データー読み込み　1.ひらがな
'1画の文字   6文字 23文字
open "config/Kanji_data/Mojidata_ReiIden20200911.dat" for input as #1
'全ファイルを読み込む
...
next i
close #1
open "config/Kikkyo_data/Anzai_Kikkyo_chiunn.dat" for input as #2
for m=0 to 69
input #2,buf_Kikkyo_Anzai_chiunn$(m)
next m
close #2
open "config/Kikkyo_data/Anzai_Kikkyo_jinunn.dat" for input as #3
for i=1 to 70
input #3,buf_Kikkyo_Anzai_jinunn$(i-1)
next i
close #3
open "config/Kikkyo_data/Anzai_Kikkyo_Total.dat" for input as #4
for i=0 to 79
...
color rgb(255,0,255):PRINT"Ver:240.2024.12.01" + chr$(13)
...
'ここを書き換える
talk "このプログラムは、姓名判断 スリー　イン　ワン、 バージョン2.40です"
...
cls
'設定ファイルの確認
if dir$("config/Parsonal_data/mydata.dat")="" then
'1.ファイルがない時
goto Kaimei_check:
else
'ファイルがある時
...
'性別変数 sex_type$
if type = 1 then
print"自分の性別(女性 or 男性):";sex_type$(1)+chr$(13):sex_type$=sex_type$(1):goto 33280
else
...
print"コマンド:"
key$=Input$(1)
if key$=chr$(13) then goto 36310
IF key$="q" then goto 7660
'
cls
color rgb(255,255,255)
...
name$=kouho_name$
n=len(name$)
if n > 10 then ui_msg"文字は10文字までです":name$ = "":kouho_name$ = "":goto 36590
'else
if n =< 10 then goto 31570
endif
...
line (0,0)-(847,57),rgb(0,255,0),b
'Line 2
line (0,63)-(853,537),rgb(127,255,212),bf
line (0,60)-(850,540),rgb(0,0,0),b
'Line 3
line (0,540)-(850,630),rgb(0,255,0),bf
line (0,543)-(853,633),rgb(0,0,255),b
'グラフィック領域　ここまで
'音声表示
...
'Title
line (0,0)-(850,60),rgb(0,0,255),bf
pen 3:line (0,0)-(850,57),rgb(255,255,255),b
'name
line (0,60)-(850,165),rgb(0,255,0),bf
pen 5:line (0,63)-(850,162),rgb(255,0,255),b
'生数、和数
line (0,165)-(850,550),rgb(125,255,212),bf
pen 3:line(0,168)-(850,547),rgb(0,0,255),b
'プッシュメッセージ
line (0,550)-(850,650),rgb(0,255,0),bf
pen 5:line (0,553)-(850,653),rgb(0,0,0),b
'描画領域　ここまで
color rgb(255,255,255)
